...
1. **逻辑查询无能为力**——普通 RAG 先用小模型生成检索词、再走向量库，面对"统计文档数量""财报中盈利最高的公司"这类跨文档聚合、需要结合数据库的计算型问题完全束手无策。
...
| 问题 | 影响 | 计划 |
|------|------|------|
| **chunk_size 偏大** | 查询返回单块内容过多（~6000 字符），冲淡关键信息；LightRAG 实体提取慢 | 下迭代缩小至 2000，并发 worker 升至 6 |
| **LLM 答案偏简洁** | 最终答案 100-200 字，缺少细节引用和深度说明 | 优化答案 prompt，移除 conciseness 约束，加入结构化思考指导 |
| **评分机制缺失** | 无法自动评估回答质量，无法量化信息完整性和幻觉风险 | 中长期建立多维评分体系（完整性/准确性/可溯源性/无幻觉），前端展示评分标签 |
| **PDF 处理感知弱** | 大文件上传后用户等待无反馈 | 已改为 202 响应 + 后台处理 + 前端轮询，仍需优化处理速度 |

### 后续迭代方向

1. **chunk_size 调优**：6000 → 2000，配合并发 worker 数提升，平衡实体提取质量与查询响应速度
2. **Prompt 优化**：答案生成加入深度思考指令，去除"简洁"约束，按问题类型灵活调整回答详细度
3. **答案评分机制**（中长期）：LLM 自动四维评分（完整性/准确性/可溯源性/无幻觉），反馈到前端
4. **更多数据源**：支持更多文件类型、外部数据库连接
5. **多轮对话记忆**：跨轮上下文保持，Agent 支持反问澄清用户需求
